...
// 查询指定物品库存
const checkItemInventory = async (conn, itemName) => {
    try {
        const [rows] = await conn.query(`SELECT * FROM inventory WHERE item_name = ?`, [itemName]);
        console.log(`\n===== 物品 "${itemName}" 库存情况 =====`);
        if (rows.length === 0) {
            console.log(`物品 "${itemName}" 不存在于库存中`);
            return null;
        }
        console.log(rows[0]);
        return rows[0];
    } catch (err) {
        console.error(`查询物品 ${itemName} 失败:`, err.message);
        return null;
    }
};
...
(async () => {
    ...
    try {
        ...
        // 1. 检查灵之精火的库存
        await checkItemInventory(conn, "灵之精火");
        
        // 2. 表结构检查可选
        // await showTableStructure(conn, 'inventory');
        
        // 3. 最近的出库记录可选
        // await getRecentRecords(conn, 'stock_out', 3);
