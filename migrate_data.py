...
class DataMigrator:
    ...
    def save_migration_log(self, tables):
        """保存迁移日志"""
        log_file = os.path.join(self.backup_dir, f"migration_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        log_data = {
            "migration_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "tables": tables,
            "source_db": "game_trading",
            "target_db": "OcrTrade",
            "target_host": "192.168.3.22"
        }
    ...
    def restore_database(self, backup_file, db_ip, user, passwd, db_name):
        if not os.path.exists(backup_file):
            ...
            return False
        print(f"\n开始从备份文件恢复数据库: {backup_file}")
        try:
            # 使用mysql命令恢复
            cmd = f'mysql -h {db_ip} -u {user} -p{passwd} {db_name} < "{backup_file}"'
            os.system(cmd)
            print("数据库恢复完成")
            return True
        except Exception as e:
