...
def fix_inventory():
    ...
    for item_name, data in inventory.items():
        ...
        # 如果库存为零，尝试使用平均出库价格作为入库价格
        if data['in_qty'] == 0 and data['out_qty'] > 0:
            avg_out_price = data['out_amount'] / data['out_qty']
            # 仅更新 in_qty 和 in_cost，不改变数量
            data['in_qty'] = data['out_qty']
            data['in_cost'] = data['out_qty'] * avg_out_price
            data['total_cost'] = data['in_cost']
            print(f"物品 '{item_name}' 无入库记录但有出库记录，已将入库均价设为 {avg_out_price:.2f}")
        ...
        # 如果库存为负，需要添加相应的入库记录
            ...
            if data['out_qty'] > 0:
                avg_out_price = data['out_amount'] / data['out_qty']
                # 补充足够的入库数量，使库存不为负
                additional_qty = abs(quantity)
                additional_cost = additional_qty * avg_out_price
                
                data['in_qty'] += additional_qty
                data['in_cost'] += additional_cost
                data['total_cost'] += additional_cost
                data['quantity'] = 0  # 将库存调整为0
                
                print(f"物品 '{item_name}' 库存为负({quantity})，已补充 {additional_qty} 个入库记录，价格为 {avg_out_price:.2f}")
